...
function promedioResultadosTest(resultadosTest) {
  ...
  // Tu código:
  product = 0;
  result = resultadosTest.length;
  for(let i=0; i < resultadosTest.length; i++) {
    product = product + resultadosTest[i];
  }
...
function todosIguales(arreglo) {
  ...
  //Escribe tu código aquí  
  let si = 0;
  let product = arreglo[0];
  for(let i=0; i < arreglo.length; i++) {
    if(product !== arreglo[i]) {
      si++
    }
  }
  if(si == 0) {
...
function mayorACien(array) {
  ...
  for(let i=0; i < array.length; i++) {
    if(array[i] >= 0 && array[i] <= 200) {
      if(array[i] > 100) {
        numerosNuevos.push(array[i]);
      }
    }
    else {
      return "Solo números enteros, mayores a 0 y menores a 200";
    }
  }
